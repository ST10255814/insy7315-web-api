...
Set-Location backend
try {
    npx eslint src/ --format json --output-file eslint-report.json --config eslint.config.js 2>$null
    Write-Host "✅ ESLint analysis completed" -ForegroundColor Green
} catch {
    Write-Host "ℹ️  ESLint analysis skipped" -ForegroundColor Cyan
}

# Run Security ESLint Analysis
Write-Host "🔐 Running Security ESLint Analysis..." -ForegroundColor Yellow
try {
    npx eslint src/ --config .eslintrc.security.js --format json --output-file eslint-security-report.json 2>$null
    Write-Host "✅ Security ESLint analysis completed" -ForegroundColor Green
} catch {
    Write-Host "ℹ️  Security ESLint analysis skipped" -ForegroundColor Cyan
}
Set-Location ..
