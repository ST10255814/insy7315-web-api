...
const validateUserData = (userData) => {
  ...
  if (!validation.validatePassword(password)) {
    throw new Error(
      "Password must be at least 8 characters with letters and numbers, and cannot contain inappropriate content"
    );
  }

  if (!validation.validateUsername(username.toLowerCase())) {
    throw new Error(
      "Username contains inappropriate content or invalid format"
    );
  }
};
...
const resetPassword = async (data) => {
  try {
    ...
    const { generateShortLivedToken } = await import("../utils/jwtUtils.js");
    const resetToken = generateShortLivedToken(
      { userId: user._id, email: emailLower },
      "10m"
    );
    ...
    await sendResetPasswordEmail(emailLower, name, process.env.RESET_PASSWORD_URL, resetToken);
...
const updatePassword = async (data, userId) => {
  try {
    ...
    // Update password in database and clear reset token with patch
    await systemUsers.updateOne(
...
const userService = { register, login, resetPassword, updatePassword };
