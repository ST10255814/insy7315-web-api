    ...
    for (const request of requests) {
      try {
        ...
        const formattedRequest = {
          maintenanceID:
            request.newMaintenanceRequest?.maintenanceId || request._id,
          ...
          createdAt:
            request.newMaintenanceRequest?.createdAt || request.createdAt,
          updatedAt:
            request.newMaintenanceRequest?.updatedAt || request.updatedAt,
        ...
        throw new Error(
          `Error processing request ${request._id}: ${requestError.message}`
        );
      }
    }
...
async function countHighPriorityMaintenanceRequestsByAdminId(adminId) {
  try {
    ...
    throw new Error(
      `Error counting high priority maintenance requests: ${error.message}`
    );
  }
}
...
async function createCareTaker(adminId, careTakerData) {
  try {
    ...
    const { firstName, surname, email, phoneNumber, profession } =
      careTakerData;
...
async function getAllAdminsCareTakers(adminId) {
  try {
    ...
    const caretakers = await userCollection
...
async function assignCareTakerToRequest(
  caretakerId,
  ...
  try {
    ...
    const { validateString, validateObjectId } = await import(
    ...
    const sanitizedMaintenanceId = maintenanceRequestId.replace(
      /[^a-zA-Z0-9_-]/g,
      ""
    );
    ...
    await maintenanceCollection.updateOne(
      ...
      {
        $set: {
          "newMaintenanceRequest.caretakerId": sanitizedCaretakerId,
          "newMaintenanceRequest.status": "In Progress",
          "newMaintenanceRequest.updatedAt": new Date(),
        },
      }
    );
    ...
    throw new Error(
      `Error assigning caretaker to maintenance request: ${error.message}`
    );
  }
}
...
async function updateMaintenanceStatusToCompleted(
  maintenanceRequestId,
  adminId
) {
  try {
    ...
    await maintenanceCollection.updateOne(
      ...
      {
        $set: {
          "newMaintenanceRequest.status": "Completed",
          "newMaintenanceRequest.updatedAt": new Date(),
        },
      }
    );
    ...
    throw new Error(
      `Error updating maintenance request status: ${error.message}`
    );
  }
}
...
async function deleteCareTaker(caretakerId, adminId) {
  try {
    ...
    if (result.deletedCount > 0) {
      try {
        const { unregisterExistingId } = await import(
          "../utils/globalIdTracker.js"
        );
        await unregisterExistingId(caretakerId);
      } catch (trackerError) {
        // Log error but don't fail the deletion
        console.error(
          `Warning: Failed to unregister caretaker ID ${caretakerId}:`,
...
async function getCareTakerById(caretakerId, adminId) {
  try {
    const db = client.db("RentWise");
    const userCollection = db.collection("Care-Takers");
    const caretaker = await userCollection.findOne({
    ...
    throw new Error(`Error retrieving caretaker: ${error.message}`);
  }
}
