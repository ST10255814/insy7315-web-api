...
import { TabWrapper, ActionButton } from "../../common/index.js";
...
export default function AddProperty() {
  ...
  // Available amenities list (mock data)
  ...
  const statusOptions = ["Available", "Occupied", "Maintenance", "Under Review"];
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setIsPending(false);
      Toast.success("Property added successfully! (Mock functionality)");
      navigate(`/dashboard/${adminId}/properties`);
    }, 2000);
  };
              ...
              >
                ...
                <div className="mt-6">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                    placeholder="e.g., 123 Main Street, City Center, NY 10001"
                    required
                  />
                </div>
              ...
              >
                ...
                >
                  {formData.imageURL.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm text-gray-400">Drag and drop images here or click the button below</p>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-4">
                  ...
                  >
                    ...
                    {formData.imageURL.length > 0 ? 'Add More Images' : 'Add Images'}
                  </label>
                  <div className="mt-2 text-center">
                    {formData.imageURL.length > 0 && (
                      <p className="text-sm text-gray-600 mb-1">
                        {formData.imageURL.length} image{formData.imageURL.length !== 1 ? 's' : ''} uploaded
                      </p>
                    )}
              ...
              >
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className={`font-medium ${formData.title ? 'text-green-600' : 'text-gray-400'}`}>
                      {formData.title || 'Not filled'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Address:</span>
                    <span className={`font-medium ${formData.address ? 'text-green-600' : 'text-gray-400'}`}>
                      {formData.address ? 'Filled' : 'Not filled'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Price:</span>
                    <span className={`font-medium ${formData.price ? 'text-green-600' : 'text-gray-400'}`}>
                      {formData.price ? `$${formData.price}` : 'Not set'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Amenities:</span>
                    <span className={`font-medium ${formData.amenities.length > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                      {formData.amenities.length || 'None'}
                    </span>
                  </div>
                </div>
              </motion.div>
